<div>
    ...
    @include('partials.site-partials.breadcrumb', ['breadcrumbs' => [
        ...
        ['label' => $certificate->numero_certificat, 'url' => route('search.details', $certificate->id)]
    ]])
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            @if(!empty($certificate->formation_data))
                                @foreach($certificate->formation_data as $key => $value)
                                    <div>
                                        <h4 class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ ucfirst($key) }}</h4>
                                        <p class="text-sm text-gray-700 dark:text-gray-300">
                                            {{ $value }}
                                        </p>
                                    </div>
        ...
        @else
            <div class="text-center py-12">
                ...
                <p class="text-gray-500 dark:text-gray-400">Le certificat demandé n'existe pas ou n'est plus
                    disponible.</p>
            </div>
        @endif
    </div>
